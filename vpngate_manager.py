...
class SlotState:
    def __init__(self, slot_id: int):
        ...
        self.latency_ms   = 0
        # 主备节点状态
        self.using_backup = False   # 当前是否在用备用节点
        self.primary_id   = ""      # 主节点ID
        self.backup_ids: list[str] = []  # 备用节点ID列表
        ...
        try:
            if tun_ip:
                subprocess.run(["ip", "route", "add", "default", "dev", tun_dev,
                                "src", tun_ip, "table", str(table_id)], check=True, timeout=3)
            else:
                subprocess.run(["ip", "route", "add", "default", "dev", tun_dev,
...
def stop_slot(slot_id: int) -> None:
    ...
    st.status_msg = "已断开"; st.using_backup = False
    srv = proxy_mod._proxy_slots.get(slot_id)
    if srv: srv.set_upstream_socks5(None)
    with lock:
        nodes = read_json(NODES_FILE, [])
        for n in nodes:
